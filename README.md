...
- **自定义远程端口**：支持自定义 XRDP 远程桌面端口（默认 3389）
- **XRDP 色深智能切换**：
...
- **Firefox ESR 浏览器**：稳定可靠的长期支持版本浏览器（自动回退到 firefox / chromium）
- **LXTerminal**：轻量级终端模拟器

### 远程桌面
- **XRDP 服务**：开箱即用的 RDP 远程桌面，支持自定义端口（默认 `3389`）
...
- **FeatherPad**：轻量级 Qt5 文本编辑器（替代已移除的 Leafpad）
...
1. **Windows 远程桌面连接（mstsc）**
   - 地址：`服务器IP:端口`（默认端口 3389，如自定义端口则使用自定义端口）
   - 使用系统现有的任意本地 Linux 账户登录（包括 root）

2. **安全组提醒**
   - 请确保云服务器控制台（安全组）已开放您设置的端口（默认 3389）
